...
class nnSeq2SeqLogger(object):
    ...
    def log(self, key, value, epoch: int):
        ...
        if key == 'PSNR':
            new_ema_pseudo_dice = self.my_fantastic_logging['ema_PSNR'][epoch - 1] * 0.9 + 0.1 * value \
                if len(self.my_fantastic_logging['ema_PSNR']) > 0 else value
            self.log('ema_PSNR', new_ema_pseudo_dice, epoch)
    ...
    def plot_progress_png(self, output_folder):
        ...
        # regular progress.png as we are used to from previous nnSeq2Seq versions
        ax = ax_all[0]
        ax2 = ax.twinx()
        ...
        ax2.plot(x_values, self.my_fantastic_logging['PSNR'][:epoch + 1], color='g', ls='dotted', label="pseudo PSNR",
                 linewidth=3)
        ax2.plot(x_values, self.my_fantastic_logging['ema_PSNR'][:epoch + 1], color='g', ls='-', label="pseudo PSNR (mov. avg.)",
                 linewidth=4)
        ax.set_xlabel("epoch")
        ax.set_ylabel("loss")
        ax2.set_ylabel("pseudo PSNR")
        ax.legend(loc=(0, 1))
