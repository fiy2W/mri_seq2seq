...
from nnseq2seq.training.dataloading.utils import get_case_identifiers


class nnSeq2SeqDataset(object):
    def __init__(self, folder: str, case_identifiers: List[str] = None,
        ...
        IMPORTANT! THIS CLASS ITSELF IS READ-ONLY. YOU CANNOT ADD KEY:VALUE PAIRS WITH nnSeq2SeqDataset[key] = value
        USE THIS INSTEAD:
        nnSeq2SeqDataset.dataset[key] = value
        ...
        self.keep_files_open = ('nnSeq2Seq_keep_files_open' in os.environ.keys()) and \
                               (os.environ['nnSeq2Seq_keep_files_open'].lower() in ('true', '1', 't'))
        # print(f'nnSeq2SeqDataset.keep_files_open: {self.keep_files_open}')
    ...
    def load_case(self, key):
        ...
        # Fill in missing sequences with zeros.
        data_stack = [np.zeros_like(data[0]) for _ in range(entry['properties']['num_channel'])]
        for i, si in enumerate(entry['properties']['available_channel']):
            data_stack[si] = data[i]
...
if __name__ == '__main__':
    ...
    folder = '/media/fabian/data/nnSeq2Seq_preprocessed/Dataset003_Liver/3d_lowres'
    ds = nnSeq2SeqDataset(folder, num_images_properties_loading_threshold=0) # this should not load the properties!
    ...
    # this should have the properties
    ds = nnSeq2SeqDataset(folder, num_images_properties_loading_threshold=1000)
    ...
    ds = nnSeq2SeqDataset(folder, num_images_properties_loading_threshold=0)
    # now rename the properties file so that it does not exist anymore
    shutil.move(join(folder, 'liver_0.pkl'), join(folder, 'liver_XXX.pkl'))
