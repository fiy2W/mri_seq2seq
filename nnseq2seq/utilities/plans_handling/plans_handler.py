...
from nnseq2seq.preprocessing.resampling.utils import recursive_find_resampling_fn_by_name
import nnseq2seq
from batchgenerators.utilities.file_and_folder_operations import load_json, join

from nnseq2seq.imageio.reader_writer_registry import recursive_find_reader_writer_by_name
from nnseq2seq.utilities.find_class_by_name import recursive_find_python_class
from nnseq2seq.utilities.label_handling.label_handling import get_labelmanager_class_from_plans
...
if TYPE_CHECKING:
    from nnseq2seq.utilities.label_handling.label_handling import LabelManager
    from nnseq2seq.imageio.base_reader_writer import BaseReaderWriter
    from nnseq2seq.preprocessing.preprocessors.default_preprocessor import DefaultPreprocessor
...
class ConfigurationManager(object):
    def __init__(self, configuration_dict: dict):
        ...
        if 'architecture' not in self.configuration.keys():
            warnings.warn("Detected old nnSeq2Seq plans format. Attempting to reconstruct network architecture "
                          "parameters. If this fails, rerun nnSeq2Seqv2_plan_experiment for your dataset. If you use a "
                          "custom architecture, please downgrade nnSeq2Seq to the version you implemented this "
    ...
    def preprocessor_class(self) -> Type[DefaultPreprocessor]:
        preprocessor_class = recursive_find_python_class(join(nnseq2seq.__path__[0], "preprocessing"),
                                                         self.preprocessor_name,
                                                         current_module="nnseq2seq.preprocessing")
...
if __name__ == '__main__':
    ...
    plans = load_json(join(nnSeq2Seq_preprocessed, maybe_convert_to_dataset_name(3), 'nnSeq2SeqPlans.json'))
