...
from nnseq2seq.networks.seq2seq.model3d.convnext import LayerNorm, AttnResBlock, hyperConv, hyperAttnResBlock
...
class ImageEncoder(nn.Module):
    def __init__(self, args) -> None:
        ...
        self.z_act = nn.LeakyReLU(0.01, inplace=True)
        c_pre = self.c_in
        up_scale = 1
    ...
    def tsf(self, z, s, weight, level, b, n):
        ...
        z = z.reshape(b, n, c0, d0, w0, h0)*s.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
    ...
    def forward(self, x, s_all, s_subgroup):
        ...
        for i in reversed(range(len(features))):
            ...
            x1 = torch.tile(self.up_layers[i](zq1).unsqueeze(1), (1,n,1,1,1,1))
            x2 = torch.tile(self.up_layers[i](zq2).unsqueeze(1), (1,n,1,1,1,1))
            d0, w0, h0 = x1.shape[3:]
            x1 = x1.reshape(-1,c0,d0,w0,h0)
            x2 = x2.reshape(-1,c0,d0,w0,h0)
