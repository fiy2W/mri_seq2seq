...
from nnseq2seq.networks.seq2seq.model3d.convnext import LayerNorm, hyperAttnResBlock, hyperConv
    ...
    def __init__(self, args) -> None:
        ...
        for i, (ce, ke, se, nr, kr, pr) in enumerate(zip(self.c_enc, self.k_enc, self.s_enc, self.n_res, self.k_res, self.p_res)):
            self.down_layers.append(
                hyperConv(self.style_dim, self.latent_space_dim if i==0 else self.latent_space_dim+ce, ce, ksize=3, padding=1, weight_dim=self.hyper_dim, ndims=3),
            )
            ...
            if i==(len(self.c_enc)-1):
                ...
                self.up_layers.append(nn.Sequential(
                    LayerNorm(ce, eps=1e-6, data_format="channels_first"),
                    nn.Conv3d(ce, out_channels=self.c_enc[i+1], kernel_size=3, padding=1, stride=1, padding_mode='zeros'),
                    nn.Upsample(scale_factor=se, mode='nearest'),
                ))
                up_scale = up_scale//se
        
        self.image_fusion = nn.Sequential(
            nn.Conv3d(self.latent_space_dim, out_channels=3, kernel_size=1, padding=0, stride=1, padding_mode='zeros'),
            nn.LeakyReLU(0.01, inplace=True),
        )
        self.image_fusion_inv = nn.Sequential(
            nn.Conv3d(3, out_channels=self.latent_space_dim, kernel_size=1, padding=0, stride=1, padding_mode='zeros'),
        )
    ...
    def forward(self, zqs, s, latent_focal=None):
        ...
        for i, (z0, down, midr, deep, up) in enumerate(zip(zqs, self.down_layers, self.midres_layers, self.deep_layers, self.up_layers)):
            if zid==0:
                ...
                z = F.interpolate(zqs[zid], size=z0.shape[2:], mode='trilinear')
            if i==0:
                x = down(z, s)
            else:
