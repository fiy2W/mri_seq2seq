...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, args):
        ...
        sequence = [nn.Conv3d(self.c_in, ndf, kernel_size=kw, stride=2, padding=padw), nn.LeakyReLU(0.2, True)]
        nc_pre = ndf
        for nc in self.c_enc[1:]:  # gradually increase the number of filters
            sequence += [
                nn.Conv3d(nc_pre, nc, kernel_size=kw, stride=2, padding=padw),
        ...
        self.conv_out = nn.Conv3d(nc, 1, kernel_size=kw, stride=1, padding=padw)  # output 1 channel prediction map
