    ...
    def __init__(self, args) -> None:
        ...
        for i, (ce, ke, se, de, nr, kr, pr) in enumerate(zip(self.c_enc, self.k_enc, self.s_enc, self.d_enc, self.n_res, self.k_res, self.p_res)):
            ...
            self.deep_layers.append(nn.Sequential(
                nn.Conv2d(ce, out_channels=self.c_out, kernel_size=7, padding=3, stride=1, padding_mode='reflect'),
        ...
        self.up_out = nn.Sequential(
            ...
            nn.Conv2d(c_pre, out_channels=c_pre//2, kernel_size=3, padding=1, stride=1, padding_mode='reflect'),
            LayerNorm(c_pre//2, eps=1e-6, data_format="channels_first"),
            nn.LeakyReLU(0.01, inplace=True),
            nn.Conv2d(c_pre//2, out_channels=self.c_out, kernel_size=7, padding=3, stride=1, padding_mode='reflect'),
            nn.LeakyReLU(0.01, inplace=True),
        )
