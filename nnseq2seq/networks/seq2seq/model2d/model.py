...
from nnseq2seq.networks.seq2seq.model2d.quantize import SoftVectorQuantizer
...
class DualSeq2Seq(nn.Module):
    def __init__(self, args) -> None:
        ...
        self.syn_output_conv_x4 = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv2d(in_channels=self.c_enc[2], out_channels=self.c_in, kernel_size=3, padding=1, stride=1)
        )
        self.seg_output_conv_x4 = nn.Sequential(
            nn.GroupNorm(num_groups=self.group, num_channels=self.c_enc[2], affine=True),
            nn.SiLU(),
            nn.Conv2d(in_channels=self.c_enc[2], out_channels=self.num_classes, kernel_size=3, padding=1, stride=1)
        )
        ...
        self.syn_output_conv_x2 = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv2d(in_channels=self.c_enc[1], out_channels=self.c_in, kernel_size=3, padding=1, stride=1)
        )
        self.seg_output_conv_x2 = nn.Sequential(
            nn.GroupNorm(num_groups=self.group, num_channels=self.c_enc[1], affine=True),
            nn.SiLU(),
            nn.Conv2d(in_channels=self.c_enc[1], out_channels=self.num_classes, kernel_size=3, padding=1, stride=1)
        )
        ...
        self.syn_output_conv_x1 = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv2d(in_channels=self.c_enc[0], out_channels=self.c_in, kernel_size=3, padding=1, stride=1)
        )
        self.seg_output_conv_x1 = nn.Sequential(
            ResBlock(self.c_enc[1]+self.c_enc[0], self.c_enc[0], stride=1, conv_shortcut=False, groups=self.group, up=False, down=False),
            nn.GroupNorm(num_groups=self.group, num_channels=self.c_enc[0], affine=True),
            nn.SiLU(),
            nn.Conv2d(in_channels=self.c_enc[0], out_channels=self.num_classes, kernel_size=3, padding=1, stride=1)
        )
...
class ResBlock(nn.Module):
    def __init__(
        ...
        if self.up:
            ...
            self.downsample = nn.AvgPool2d(kernel_size=self.stride, stride=self.stride)
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.fusion_conv_syn = nn.Conv2d(self.c_enc[2]*2, self.latent_dim, kernel_size=3, stride=1, padding=1)
        self.fusion_conv_seg = nn.Conv2d(self.c_enc[2]*2, self.latent_dim, kernel_size=3, stride=1, padding=1)
        ...
        self.fusion_deconv_syn = nn.Conv2d(self.latent_dim, self.c_enc[2], kernel_size=3, stride=1, padding=1)
        self.fusion_deconv_seg = nn.Conv2d(self.latent_dim, self.c_enc[2], kernel_size=3, stride=1, padding=1)
