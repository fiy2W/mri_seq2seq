...
class SoftVectorQuantizer(nn.Module):
    ...
    def forward(self, z):
        ...
        # Reshape back
        z_q_soft = z_q_soft.view(batch_size, h, w, self.e_dim).contiguous()
        z_q_hard = z_q_hard.view(batch_size, h, w, self.e_dim).contiguous()
        ...
        if len(z.shape) == 4:
            z_q_soft = torch.einsum('b h w c -> b c h w', z_q_soft)
            z_q_hard = torch.einsum('b h w c -> b c h w', z_q_hard)
    ...
    def get_codebook_entry(self, indices):
        ...
        z_q_hard = z_q_hard.view(b, h, w, self.e_dim).contiguous()
        z_q_hard = torch.einsum('b h w c -> b c h w', z_q_hard)
        return z_q_hard
