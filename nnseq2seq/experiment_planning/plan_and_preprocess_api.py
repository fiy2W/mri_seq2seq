...
import nnseq2seq
from nnseq2seq.experiment_planning.dataset_fingerprint.fingerprint_extractor import DatasetFingerprintExtractor
from nnseq2seq.experiment_planning.experiment_planners.default_experiment_planner import ExperimentPlanner
from nnseq2seq.experiment_planning.verify_dataset_integrity import verify_dataset_integrity
from nnseq2seq.paths import nnSeq2Seq_raw, nnSeq2Seq_preprocessed
from nnseq2seq.utilities.dataset_name_id_conversion import convert_id_to_dataset_name
from nnseq2seq.utilities.find_class_by_name import recursive_find_python_class
from nnseq2seq.utilities.plans_handling.plans_handler import PlansManager
...
def extract_fingerprint_dataset(
    ...
    if check_dataset_integrity:
        verify_dataset_integrity(join(nnSeq2Seq_raw, dataset_name), num_processes)
...
def extract_fingerprints(
    ...
    """
    clean = False will not actually run this. This is just a switch for use with nnseq2seq_plan_and_preprocess where
    we don't want to rerun fingerprint extraction every time.
    """
    fingerprint_extractor_class = recursive_find_python_class(join(nnseq2seq.__path__[0], "experiment_planning"),
                                                              fingerprint_extractor_class_name,
                                                              current_module="nnseq2seq.experiment_planning")
...
def plan_experiment_dataset(dataset_id: int,
    ...
    """
    overwrite_target_spacing ONLY applies to 3d and 3d_cascade fullres!
    """
    kwargs = {}
...
def plan_experiments(dataset_ids: List[int], experiment_planner_class_name: str = 'ExperimentPlanner',
    ...
    """
    overwrite_target_spacing ONLY applies to 3d!
    """
    experiment_planner = recursive_find_python_class(join(nnseq2seq.__path__[0], "experiment_planning"),
...
def preprocess_dataset(dataset_id: int,
                       ...
                       configurations: Union[Tuple[str], List[str]] = ('2d', '3d'),
                       num_processes: Union[int, Tuple[int, ...], List[int]] = (8, 4),
    ...
    # copy the gt to a folder in the nnSeq2Seq_preprocessed so that we can do validation even if the raw data is no
    # longer there (useful for compute cluster where only the preprocessed data is available)
    from distutils.file_util import copy_file
    maybe_mkdir_p(join(nnSeq2Seq_preprocessed, dataset_name, 'gt_segmentations'))
    dataset_json = load_json(join(nnSeq2Seq_raw, dataset_name, 'dataset.json'))
    ...
    for k in dataset:
        copy_file(dataset[k]['label'],
                  join(nnSeq2Seq_preprocessed, dataset_name, 'gt_segmentations', k + dataset_json['file_ending']),
                  update=True)
