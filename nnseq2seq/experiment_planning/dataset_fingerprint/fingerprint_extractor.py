...
class DatasetFingerprintExtractor(object):
    def __init__(self, dataset_name_or_id: Union[str, int], num_processes: int = 8, verbose: bool = False):
        ...
        self.input_folder = join(nnSeq2Seq_raw, dataset_name)
    ...
    def analyze_case(image_files: List[str], segmentation_file: str, reader_writer_class: Type[BaseReaderWriter],
        ...
        available_channel = [np.int64(os.path.basename(fname).split('_')[-1].split('.')[0]) for fname in image_files]
        ...
        return shape_after_crop, spacing, foreground_intensities_per_channel, foreground_intensity_stats_per_channel, \
               relative_size_after_cropping, available_channel
    ...
    def run(self, overwrite_existing: bool = False) -> dict:
        ...
        # reliably write in nnSeq2Seq_preprocessed and nnSeq2Seq_results, so nnSeq2Seq_preprocessed it is
        preprocessed_output_folder = join(nnSeq2Seq_preprocessed, self.dataset_name)
        ...
        if not isfile(properties_file) or overwrite_existing:
            ...
            foreground_intensities_per_channel = [[] for _ in range(self.dataset_json['numChannel'])]
            for r in results:
                for ava_id, ava_channel in enumerate(r[5]):
                    foreground_intensities_per_channel[ava_channel].append(r[2][ava_id])
            foreground_intensities_per_channel = [np.concatenate(r) for r in foreground_intensities_per_channel]
